...
/* F(X) TO ADD RESOUCE RECORD */
void addResRec(Trie *node, char *rec, uint32_t *ttlMin, uint16_t *rclass)
{
	int i, seg, c;
	uint32_t ttl;
	uint16_t class;
	...
	for(i = 0; buff2 != NULL; i++)
	{
		...
		memset(buff[i], '\0',  strlen(buff2)+1);
		buff[i] = strdup(buff2);
		//printf("%s\n",buff[i]);
		buff2 = strtok(NULL, ",");
	}
	...
	switch((DnsType) type)
	{
		case a:
			...
			ars->rclass =		class;
			ars->ttl =		ttl;
			ars->rdlen = 		sizeof(struct in_addr);
			ars->anxt =		NULL;
			aptr = ars;
			break;
		case ns:
			nsrs =		(NS *) malloc(sizeof(NS));
			nsrs->nsdname =		strdup(buff[c]);
			nsrs->rclass =		class;
			nsrs->ttl =		ttl;
...
printf("%08x\n", nsptr);
		...
		case cname:
			...
			cnamers->cname =	strdup(buff[c]);
			cnamers->rclass =	class;
			cnamers->ttl =		ttl;
			cnamers->rdlen = 	strlen(cnamers->cname) + 1;
			node->val->cnamers =	cnamers;
			break;
		case soa:
			node->val->soars =			(SOA *) malloc(sizeof(SOA));
			node->val->soars->mname =		strdup(buff[c]);
			node->val->soars->rname =		strdup(buff[c+1]);
			node->val->soars->serial =		(uint32_t) atoi(buff[c+2]);
			node->val->soars->refresh =	(int32_t) atoi(buff[c+3]);
			node->val->soars->retry =		(int32_t) atoi(buff[c+4]);
			node->val->soars->expire =		(int32_t) atoi(buff[c+5]);
			node->val->soars->minimum =	(uint32_t) atoi(buff[c+6]);
			node->val->soars->rclass =		class;
			node->val->soars->rdlen =		strlen(node->val->soars->mname) + 1 +
							strlen(node->val->soars->rname) + 1 +
							sizeof(node->val->soars->serial) +
							sizeof(node->val->soars->refresh) +
							sizeof(node->val->soars->expire) +
							sizeof(node->val->soars->minimum);
			(*ttlMin) = (uint32_t) atoi(buff[c+6]);
			(*rclass) = class;
			//node->val->soars =	soars;
			break;
		case ptr:
			ptrrs =		(PTR *) malloc(sizeof(PTR));
			ptrrs->ptrdname =	strdup(buff[c]);
			ptrrs->rclass =	class;
			ptrrs->ttl =		ttl;
			ptrrs->rdlen = 	strlen(ptrrs->ptrdname) + 1;
			node->val->ptrrs =	ptrrs;
			break;
		case mx:
			mxptr = node->val->mxrs;
			if(mxptr != NULL)
		...
		case aaaa:
			...
			aaaars->rclass =	class;
			aaaars->ttl =		ttl;
			aaaars->rdlen = 	sizeof(struct in6_addr);
			aaaars->aaaanxt = NULL;
			aaaaptr = aaaars;
			break;
		default:
...
/* F(X) TO ADD TO TRIE */
void addTrie(Trie *root, char *name, char *rec, uint32_t *ttlMin, uint16_t *rclass)
{
	Trie *plc = NULL;
	int i = 0;
	int stl;
	...
	if(plc->cdn == NULL)
	{
		for(i = 0; i <= stl; i++)
		{
			if(name[i+1] == '\0')
			{
				plc->cdn = createNode(name[i]);
				addResRec(plc->cdn, rec, ttlMin, rclass);
				plc->cdn->par = plc;
				plc =  plc->cdn;
				//addResRec(plc, rec, ttlMin, rclass);
			}
			else 
			{
				plc->cdn = createNode(name[i]);
...
printf("\t\t%08x\n", plc->val);
				...
				{
					plc->snt = createNode(name[i]);
					addResRec(plc->snt, rec, ttlMin, rclass);
					plc->snt->spv = plc;
					plc = plc->snt;
					//addResRec(plc, rec, ttlMin, rclass);
printf("%s\n", name);
...
//	//uint16_t dClass = 0; //default class gets redefined by SOA
//	//RR *rrs;
//	//printf("#include \"triez.c\"\n");
//	//printf("#include \"triez.h\"\n\n");
